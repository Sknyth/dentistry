...
import { translateDoctorField } from '@/lib/doctorTranslations'
    ...
    <main className="min-h-screen px-4 py-8 md:px-24 md:py-16 flex flex-col gap-8 max-w-5xl mx-auto">
      ...
      <section className="flex flex-col gap-4">
        <div className="flex flex-col gap-1 md:flex-row md:items-center md:gap-4">
          ...
          <p className="text-gray-500 text-sm md:text-lg">{translateDoctorField('desc', doctor.id, locale, doctor.desc)}</p>
        </div>
        ...
        {doctor.bio && (
          <p className="text-gray-600 text-sm md:text-base leading-relaxed">{translateDoctorField('bio', doctor.id, locale, doctor.bio)}</p>
        )}
      </section>

      {doctor.procedures.length > 0 && (
        <section className="flex flex-col gap-3">
          <h2 className="text-xl md:text-3xl font-bold text-[#8900F2]">{t('procedures')}</h2>
          <div className="bg-[#8900F2] p-6 md:p-16 rounded-2xl w-full">
            <ul className="list-disc list-inside text-white text-sm md:text-lg flex flex-col gap-2">
              {doctor.procedures.map((p, i) => (
                <li key={i}>{translateProcedure(p, locale)}</li>
              ))}
            </ul>
          </div>
        </section>
      ...
      <section className="flex flex-col gap-4 items-center justify-center border-2 border-[#8900F2] p-8 md:p-28 rounded-2xl w-full">
        <h2 className="text-2xl md:text-4xl text-[#8900F2] font-bold text-center">{t('cta')}</h2>
