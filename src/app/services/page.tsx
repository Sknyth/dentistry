...
const categories = [
  {
    ...
    services: [
      ...
      { name: "Radiografie retroalveolară", price: "de la 15 €" },
      { name: "Pachet Igienizare Completă (Detartraj cu ultrasunete, Periaj profesional, Airflow)", price: "de la 90 €" },
      { name: "Fluorizare (pe ambele arcade)", price: "de la 40 €" },
    ]
  },
  {
    title: "Odontologie (Tratamentul Cariilor)",
    services: [
      { name: "Obturaţie fizionomică (Plombă) – mică", price: "de la 40 €" },
      { name: "Obturaţie fizionomică (Plombă) – medie/mare", price: "de la 60 €" },
    ...
    <main className="min-h-screen flex flex-col px-16 py-12 md:py-20">
      <div className="border-[#8900F2] border-3 rounded-2xl">
        ...
        <section className="p-12 flex flex-col gap-6">
          ...
          {categories.map((cat) => (
            <Service key={cat.title} title={cat.title} services={cat.services} />
          ))}
        </section>
      </div>
    </main>
